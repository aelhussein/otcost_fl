...
class OTCost:
    ...
    def feature_cost(self, i, index_1, index_2):
        if not self.private:
            ...
            self.feature_costs.append((i, np.min(feature_cost), np.percentile(feature_cost, 1), np.percentile(feature_cost, 50)))
        else:
    ...
    def total_cost(self):
        ...
        for i in label_perm:
            ...
            cost = (self.lam[0]*feat_cost)/2 + self.lam[1]*label_cost # done this way so it is lam[0] and lam[1] which scale the cost sizes rahter than fix at 2:1
    ...
    def calculate_ot_cost(self):
        ...
        self.Gs = ot.bregman.sinkhorn_stabilized(a, b, costs_stable, self.eps, stopThr=1e-3, numItermax=20000, warn = True, verbose=False)
